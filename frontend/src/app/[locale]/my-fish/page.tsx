          ...
          <>
                  ...
                  <tbody>
                    {fishItems.map((f) => {
                      ...
                      return (
                        <tr
                          key={f.fishId}
                          onClick={() => router.push(`/fish-tank/${f.tankId}`)}
                          className="border-b border-glass-border/30 hover:bg-glass/50 transition cursor-pointer"
                        >
            ...
            <div className="sm:hidden space-y-2">
              {fishItems.map((f) => {
                ...
                const statusCfg = STATUS_CONFIG[status] ?? STATUS_CONFIG.healthy;
                return (
                  <Link href={`/fish-tank/${f.tankId}`} key={f.fishId} className="block">
